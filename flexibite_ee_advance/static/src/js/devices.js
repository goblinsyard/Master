odoo.define('flexibite_ee_advance.devices', function (require) {
	"use strict";
    	...
    	scan: function(code){
    		...
    		if(current_screen == 'login' && self.pos.is_rfid_login){
    			...
    			if(user){
                	...
                	var img = "<img src='/flexibite_ee_advance/static/src/img/scan_rfid_red.png' style='height:285px;width:auto;'/>";
                	$("#image").html(img)
                	setTimeout(function(){ 
                		var img = "<img src='/flexibite_ee_advance/static/src/img/scan_rfid.png' style='height:285px;width:auto;'/>";
                        $("#image").html(img)
                	}, 1000);
                }
    		} else {
