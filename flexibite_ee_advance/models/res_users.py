...
class ResUsers(models.Model):
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        ...
        return super(ResUsers, self).name_search(name,
    ...
    default_pos_ids = fields.Many2many('pos.config', 'default_pos_config_rel',
        string="Default Point of Sale(s)")
...
#     access_change_stock_locations = fields.Boolean("Change Stock Locations", default=True)
    ...
    @api.onchange('login_with_pos_screen', 'default_pos', 'user_role', 'default_pos_ids')
    def onchange_pos_config(self):
        # User role is only available flexibite and related modules
        if self.user_role == 'cook':
            self.login_with_pos_screen = True
        if self.user_role == 'delivery_user':
        ...
        # By Devang:-
        # TODO: Don't merge below condition as '...else...' part in above one.
        # This one is kept seperate for a purpose.
        if self.login_with_pos_screen:
            self.default_pos_ids = [[6, 0, []]]
