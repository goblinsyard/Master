...
class aspl_gift_card(models.Model):
    ...
    def _send_mail_balance_and_expired_coupon(self, expired=False, balance=False):
        ...
        template_id = self.env['ir.model.data'].get_object_reference('flexibite_ee_advance',
                                                   'email_template_for_coupon_expire_7')
        balance_template_id = self.env['ir.model.data'].get_object_reference('flexibite_ee_advance',
    ...
    def write(self, vals):
        ...
        if vals.get('card_no'):
            ...
            if old_card_no != vals.get('card_no'):
                ...
                self.env['aspl.gift.card.exchange.history'].create({'old_card_no':old_card_no,'new_card_no':new_card.card_no,'customer_id':new_card.customer_id.id})
            try:
                template_id = self.env['ir.model.data'].get_object_reference('flexibite_ee_advance',
...
class aspl_gift_card_use(models.Model):
    ...
    _description = 'aspl gift card use'
    _order = 'id desc'
...
class aspl_gift_card_recharge(models.Model):
    ...
    _rec_name = 'amount'
    _description = 'aspl gift card recharge'
    _order = 'id desc'
...
class aspl_gift_card_type(models.Model):
    ...
    _rec_name = 'name'
    _description = 'aspl gift card type'
