...
class Home(Home):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if request.params['login_success']:
            ...
            if user.login_with_pos_screen or user.user_role == 'cook':
    ...
    def send_pos_ordermail(self, **kw):
        ...
        for order in orders:
            if order.partner_id.email and order.session_id.config_id.enable_ereceipt:
                try:
                    template_id = request.env['ir.model.data'].get_object_reference('flexibite_ee_advance', 'email_template_pos_ereceipt')
...
class PosMirrorController(http.Controller):
    ...
    def white_board_web(self, **k):
        ...
        return request.render('flexibite_ee_advance.customer_display_index', qcontext=context)
