...
class NonMovingStock(models.TransientModel):
    ...
    _description = 'Pos Non Moving Stock'
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(NonMovingStock, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar,
                                                          submenu=submenu)
        if not self.env['ir.config_parameter'].sudo().get_param('flexibite_ee_advance.groups_ids'):
            if view_type == 'form':
    ...
    def print_report(self):
        ...
        if self.report_type == 'pdf':
            if self.env.context.get('send_mail'):
                ...
                template_id = self.env.ref('flexibite_ee_advance.non_moving_stock_report')
                ...
                vals_new = {'products': final_non_moving_data}
                return self.env.ref('flexibite_ee_advance.non_moving_stock_report').report_action(self, data=vals_new)
            ...
            if self.group_by_category == True:
                for j in vals:
                    ...
                    for dict in vals.get(j):
                        ...
                        worksheet.write(row, 4,str('%.2f' % (dict.get('unit_price'))) + str(self.env.user.company_id.currency_id.symbol), horz_style_pc)
                        worksheet.col(5).width = 4500
                        worksheet.write(row, 5,str('%.2f' % (dict.get('sell_price'))) + str(self.env.user.company_id.currency_id.symbol),
                                        horz_style_pc)
                        row = row + 1
                    row = row + 1
            else:
    ...
    def send_mail(self):
        ...
        groups_ids = ast.literal_eval(
            self.env['ir.config_parameter'].sudo().get_param('flexibite_ee_advance.groups_ids'))
        for each in groups_ids:
            ...
            try:
                template_id = self.env.ref('flexibite_ee_advance.send_mail_non_moving_stock_report')
            except Exception:
...
class ResConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        if get_param('flexibite_ee_advance.groups_ids'):
            res.update(
                groups_ids=ast.literal_eval(get_param('flexibite_ee_advance.groups_ids'))
            )
        return res
    ...
    def set_values(self):
        ...
        set_param('flexibite_ee_advance.groups_ids', self.groups_ids.ids)
