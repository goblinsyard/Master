odoo.define('flexibite_ee_advance.popup', function (require) {
	"use strict";
...
//                window.document.body.removeEventListener('keypress',self.pos.gui.screen_instances.payment.keyboard_handler);
//                window.document.body.removeEventListener('keydown',self.pos.gui.screen_instances.payment.keyboard_keydown_handler);
            });
...
//                window.document.body.addEventListener('keypress',self.pos.gui.screen_instances.payment.keyboard_handler);
//                window.document.body.addEventListener('keydown',self.pos.gui.screen_instances.payment.keyboard_keydown_handler);
            });
...
//            		                    selectedOrder.add_orderline(line);
	    			...
	    			console.log("to_be_return",to_be_return)
...
//                $('.js_reservation_mode').removeClass('highlight');
    ...
    var ReportPopupWidget = PopupWidget.extend({
        ...
        session_report_pdf: function(e){
        	console.log("session_report_pdf call");
        	var self = this;
        	var session_id = $(e.currentTarget).data('id');
        	self.pos.chrome.do_action('flexibite_ee_advance.report_pos_inventory_session_pdf_front',{additional_context:{
        ...
        session_report_thermal: function(e){
        	console.log("session_report_thermal call");
        	var self = this;
        	var session_id = $(e.currentTarget).data('id');
        	var report_name = "flexibite_ee_advance.front_inventory_session_thermal_report_template";
            var params = {
        ...
        location_report_pdf: function(e){
        	console.log("location_report_pdf call");
        	var self = this;
        	var location_id = $(e.currentTarget).data('id');
        	self.pos.chrome.do_action('flexibite_ee_advance.report_pos_inventory_location_pdf_front',{additional_context:{
        ...
        location_report_thermal: function(e){
        	console.log("location_report_thermal call");
        	var self = this;
        	var location_id = $(e.currentTarget).data('id');
        	var report_name = "flexibite_ee_advance.front_inventory_location_thermal_report_template";
            var params = {
				model: 'ir.actions.report',
				method: 'get_html_report',
	    ...
	    click_confirm: function(){
            ...
            rpc.query(params, {async: false}).then(function(vals){
                if(vals){
                	...
                	console.log("vals.amount_due",vals.amount_due);
                	var partner = self.pos.db.get_partner_by_id(partner_id);
