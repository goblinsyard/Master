...
class pos_store(models.Model):
    _name = 'pos.shop'
    _description = 'Pos Shop'

    name = fields.Char(string='Name',required=True)
    image = fields.Binary(string='Image')
    store_manager = fields.Many2one("res.users",string="Shop Manager")
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        if self._context.get('company_ids'):
            ...
            args += [('id', 'in', shop_ids)]
        return super(pos_store, self).name_search(name, args=args, operator=operator, limit=limit)
