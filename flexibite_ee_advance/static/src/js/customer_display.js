odoo.define('flexibite_ee_advance.customer_display', function (require) {
"use strict";
...
//	        Right Panel
			...
			this.call('bus_service', 'updateOption','customer.display',session.uid);
	    ...
	    _onNotification: function(notifications){
	    	...
	    	for (var notif of notifications) {
	    		if(notif[1].customer_display_data){
	    			...
	    			var is_new_order = notif[1].customer_display_data.new_order;
	    			if(!is_new_order){
		                ...
		                if(payment_info && payment_info.length > 0){
		                	var payment_total_amount = 0;
		                	payment_info.map(function(payment_line){
		                		payment_total_amount += payment_line.amount;
		                	});
		                	if(order_total && payment_total_amount){
		                		change_amount = payment_total_amount - order_total;
		                	}
		                }
	    				...
	    				self.render_customer();
	    				self.left_panel.update_cart_data('');
	    				self.right_panel.update_data(0, 0, []);
