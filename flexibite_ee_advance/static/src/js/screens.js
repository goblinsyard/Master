odoo.define('flexibite_ee_advance.screens', function (require) {
	"use strict";
...
//	        	order.set_resturant_mode("dine_in_mode",this);
	        ...
	        $('div#take_away').click(function(){
                ...
                if (orderLines.length != 0) {
                	var selected_line = order.get_selected_orderline();
                	if (selected_line) {
                        if(selected_line.get_take_away()){
                        	selected_line.set_take_away(false);
                        	if(selected_line.modifier_line && selected_line.modifier_line.length > 0){
                    			var modifier_list = selected_line.modifier_line;
                    			_.each(modifier_list, function(mod_line){
                    				mod_line['is_takeaway'] = false;
                    			});
                    		}
                        } else {
...
//		    	}
    	...
    	show: function(){
            ...
            $('#wrapper1').addClass('toggled');
    		var order = this.pos.get_order();
//    		if(order.get_restuarant_mode()){
//    			order.set_resturant_mode(order.get_restuarant_mode());
//    		} else{
//    			if(self.pos.config.pos_type){
////                	order.set_restuarant_mode(self.pos.config.pos_type);
//    				if(self.pos.config.pos_type == "drive_through_mode"){
//    					$('div#drive_through_mode').trigger('click');
//    				} else if(self.pos.config.pos_type == "dine_in_mode"){
//    					$('div#dine_in_mode').trigger('click');
//    				} else if(self.pos.config.pos_type == "online_mode"){
        ...
        click_proceed: function(){
        	...
        	if(this.variant_list_widget.variant_info.length > 0){
        		this.variant_list_widget.variant_info.map(function(variant){
        			...
        			if(product){
        				self.pos.get_order().add_product(product,{'quantity':variant.qty});
        				if(variant.modifiers && variant.modifiers.length > 0){
        					...
        					if(selected_line){
        						...
        						variant.modifiers.map(function(modifier){
        							...
        							self.pos.chrome.screens.products.order_widget.orderline_change(selected_line);
        						});
        					}
        				}
        			}
        ...
        update_variant_cart: function(variant){
    		...
    		if(order && variant){
    			...
    			if(self.variant_info && self.variant_info.length > 0){
    				...
    				$('.modifiers-list-contents').html(line_html);
    			}
    		}else{
