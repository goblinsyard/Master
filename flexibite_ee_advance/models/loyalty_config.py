...
class LoyaltyConfiguration(models.TransientModel):
    ...
    def load_loyalty_config_settings(self):
        record = {}
        settings_minimum_purchase = self.env['ir.config_parameter'].sudo().search([('key', '=', 'flexibite_ee_advance.minimum_purchase')])
        if settings_minimum_purchase:
            record['minimum_purchase'] = float(settings_minimum_purchase.value)
        settings_points_based_on = self.env['ir.config_parameter'].sudo().search([('key', '=', 'flexibite_ee_advance.points_based_on')])
        if settings_points_based_on:
            record['points_based_on'] = settings_points_based_on.value
        settings_point_calculation = self.env['ir.config_parameter'].sudo().search([('key', '=', 'flexibite_ee_advance.point_calculation')])
        if settings_point_calculation:
            record['point_calculation'] = float(settings_point_calculation.value)
        settings_points = self.env['ir.config_parameter'].sudo().search([('key', '=', 'flexibite_ee_advance.points')])
        if settings_points:
            record['points'] = float(settings_points.value)
        settings_to_amount = self.env['ir.config_parameter'].sudo().search([('key', '=', 'flexibite_ee_advance.to_amount')])
    ...
    def get_values(self):
        ...
        res.update({
            'points_based_on': param_obj.sudo().get_param('flexibite_ee_advance.points_based_on'),
            'minimum_purchase': float(param_obj.sudo().get_param('flexibite_ee_advance.minimum_purchase')),
            'point_calculation': float(param_obj.sudo().get_param('flexibite_ee_advance.point_calculation')),
            'points': int(param_obj.sudo().get_param('flexibite_ee_advance.points')),
            'to_amount': float(param_obj.sudo().get_param('flexibite_ee_advance.to_amount')),
        })
        return res
    ...
    def set_values(self):
        ...
        param_obj = self.env['ir.config_parameter']
        param_obj.sudo().set_param('flexibite_ee_advance.points_based_on', self.points_based_on)
        param_obj.sudo().set_param('flexibite_ee_advance.minimum_purchase', float(self.minimum_purchase))
        param_obj.sudo().set_param('flexibite_ee_advance.point_calculation', float(self.point_calculation))
        param_obj.sudo().set_param('flexibite_ee_advance.points', int(self.points))
        param_obj.sudo().set_param('flexibite_ee_advance.to_amount', float(self.to_amount))
        return res
